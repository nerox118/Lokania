    ...
    if (isset($_POST['register'])) {
    	...
    	if ($result->num_rows == 0) {
    		if ($_POST['password'] == $_POST['confirm_password']) {
    			$result = $conn->query("SELECT * FROM user WHERE email='$_POST[email]'");
    			if ($result->num_rows == 0) {
    				$result = $conn->query("INSERT INTO user (username, password, name, email)
			...
			<div style="max-width: 500px; margin: auto;">
				...
				<form method="POST" action="" id="form-register">
					<div id="register-box">
						...
						<p>Email<a class="required">*</a></p>
						<p class="text-center">:</p>
						<input class="form-control" style="min-width: 100px" type="email" name="email" required>
						<?php
							if (isset($message)) {
								echo '<p class="text-center" style="grid-column: 1/4">'.$message.'</p>';
							}
						?>
						<button class="submit" style="grid-column: 1/4" type="submit" name="register">Register</button>
